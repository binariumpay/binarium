...
### Send 1000 BINARIUM to New Addresses

Send exactly 1000 BINARIUM to each new address created above.
...
Copy the masternode private key and correspondig collateral output transaction that holds the 1000 BINARIUM.

*Note: The masternode priviate key is **not** the same as a wallet private key. **Never** put your wallet private key in the masternode.conf file. That is almost equivalent to putting your 1000 BINARIUM on the remote server and defeats the purpose of a hot/cold setup.*
...
[From the binarium github repo](https://github.com/binariumpay/binarium/blob/master/doc/masternode_conf.md)
...
## Update binarium.conf on server

If you generated a new masternode private key, you will need to update the remote `binarium.conf` files.

Shut down the daemon and then edit the file.

```nano .binariumcore/binarium.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `binarium.conf` file.
...
```binarium-cli getinfo```

and comparing with the official explorer at https://binarium.money/explorer
