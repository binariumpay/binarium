...
void CMasternodeSync::ProcessTick(CConnman& connman)
{
    ...
    std::vector<CNode*> vNodesCopy = connman.CopyNodeVector();
    LogPrintf("CMasternodeSync::ProcessTick -- iNodesAmount %d \n", vNodesCopy.size ());
    ...
    BOOST_FOREACH(CNode* pnode, vNodesCopy)
    {
        ...
        if(Params().NetworkIDString() == CBaseChainParams::REGTEST)
        {
            ...
            connman.ReleaseNodeVector(vNodesCopy);
            LogPrintf("CMasternodeSync::ProcessTick : if(Params().NetworkIDString() == CBaseChainParams::REGTEST).\n");
            return;
        }

        // NORMAL NETWORK MODE - TESTNET/MAINNET
        {
            LogPrintf("CMasternodeSync::ProcessTick : Main Network.\n");

            // MASTERNODE_SYNC_INITIAL
            LogPrintf("CMasternodeSync::ProcessTick : Main Network : 2 : MASTERNODE_SYNC_INITIAL.\n");
            if(nRequestedMasternodeAssets == MASTERNODE_SYNC_INITIAL) {
                LogPrintf("CMasternodeSync::ProcessTick : if(nRequestedMasternodeAssets == MASTERNODE_SYNC_INITIAL).\n");
                if(GetTime() - nTimeLastBumped > MASTERNODE_SYNC_TIMEOUT_SECONDS) {
                    SwitchToNextAsset(connman);
                    LogPrintf("CMasternodeSync::ProcessTick : SwitchToNextAsset(connman).\n");
                    return;
                }
            }
            ...
            LogPrintf("CMasternodeSync::ProcessTick : Main Network : 2 : Spork.\n");
            ...
            // INITIAL TIMEOUT
            LogPrintf("CMasternodeSync::ProcessTick : Main Network : 2 : Initial timeout.\n");
            if(nRequestedMasternodeAssets == MASTERNODE_SYNC_WAITING) {
                LogPrintf("CMasternodeSync::ProcessTick : if(nRequestedMasternodeAssets == MASTERNODE_SYNC_WAITING) : %i, %i, %i.\n",
                    GetTime(), nTimeLastBumped, MASTERNODE_SYNC_TIMEOUT_SECONDS);
                if(GetTime() - nTimeLastBumped > MASTERNODE_SYNC_TIMEOUT_SECONDS) {
            ...
            LogPrintf("CMasternodeSync::ProcessTick : Main Network : 2 : MNLIST : SYNC MASTERNODE LIST FROM OTHER CONNECTED CLIENTS.\n");
            ...
            LogPrintf("CMasternodeSync::ProcessTick : Main Network : 2 : MNLIST : SYNC MASTERNODE PAYMENT VOTES FROM OTHER CONNECTED CLIENTS.\n");
            if(nRequestedMasternodeAssets == MASTERNODE_SYNC_MNW) {
                ...
                if(netfulfilledman.HasFulfilledRequest(pnode->addr, "masternode-payment-sync")){
                    LogPrintf("CMasternodeSync::ProcessTick : if(netfulfilledman.HasFulfilledRequest(pnode->addr, \"masternode-payment-sync\")).\n");
                    continue;
                }
                ...
                if(pnode->nVersion < mnpayments.GetMinMasternodePaymentsProto()){
                    LogPrintf("CMasternodeSync::ProcessTick : if(pnode->nVersion < mnpayments.GetMinMasternodePaymentsProto()).\n");
                    continue;
                }
            ...
            LogPrintf("CMasternodeSync::ProcessTick : Main Network : 2 : GOVOBJ : SYNC GOVERNANCE ITEMS FROM OUR PEERS.\n");
    ...
    LogPrintf("CMasternodeSync::ProcessTick : connman.ReleaseNodeVector(vNodesCopy).\n\n");
    connman.ReleaseNodeVector(vNodesCopy);
}
