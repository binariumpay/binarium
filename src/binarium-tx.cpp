...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc<2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help"))
    {
        ...
        std::string strUsage = _("Binarium Core binarium-tx utility version") + " " + FormatFullVersion() + "\n\n" +
            _("Usage:") + "\n" +
              "  binarium-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded binarium transaction") + "\n" +
              "  binarium-tx [options] -create [commands]   " + _("Create hex-encoded binarium transaction") + "\n" +
              "\n";
...
static void MutateTxSign(CMutableTransaction& tx, const string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        BOOST_FOREACH(const CTransaction& txv, txVariants) {
            txin.scriptt script via GetScriptForDestination()
    CScript scriptPubKey = GetScriptForDestination(addr.Get());
