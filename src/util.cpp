...
const char * const BITCOIN_CONF_FILENAME = "binarium.conf";
const char * const BITCOIN_PID_FILENAME = "binariumd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "binarium" is a composite category enabling all Binarium-related debug output
            if(ptrCategory->count(string("binarium"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\BinariumCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\BinariumCore
    // Mac: ~/Library/Application Support/BinariumCore
    // Unix: ~/.binariumcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "BinariumCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/BinariumCore";
#else
    // Unix
    return pathRet / ".binariumcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty binarium.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override binarium.conf
