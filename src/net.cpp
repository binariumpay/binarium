...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Binarium users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Binarium Core " + FormatFullVersion();
...
void CConnman::ThreadOpenAddedConnections()
{
    ...
    for (unsigned int i = 0; true; i++)
    {
        ...
        if (!interruptNet.sleep_for(boost::chrono::minutes(2)))
            return;
    }
}
...
void CConnman::ThreadMnbRequestConnections()
{
    ...
    while (!interruptNet)
    {
        if (!interruptNet.sleep_for(boost::chrono::milliseconds(1000)))
            return;
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Binarium Core is probably already running."), addrBind.ToString());
        else
...
bool CConnman::Start(CScheduler& scheduler, std::string& strNodeError, Options connOptions)
{
    ...
    threadMnbRequestConnections = boost::thread(&TraceThread<std::function<void()> >, "mnbcon", std::function<void()>(std::bind(&CConnman::ThreadMnbRequestConnections, this)));
