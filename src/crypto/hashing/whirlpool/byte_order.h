...
/* GCC >= 4.3 or clang */
# define bo_bswap_32(x) __builtin_bswap32(x)
# define bo_bswap_64(x) __builtin_bswap64(x)
#elif (_MSC_VER > 1300) && (defined(CPU_IA32) || defined(CPU_X64)) /* MS VC */
# define bo_bswap_32(x) _byteswap_ulong((unsigned long)x)
# define bo_bswap_64(x) _byteswap_uint64((__int64)x)
#else
/* fallback to generic bswap definition */
static RHASH_INLINE uint32_t bo_bswap_32(uint32_t x)
{
...
static RHASH_INLINE uint64_t bo_bswap_64(uint64_t x)
{
	union {
		uint64_t ll;
		uint32_t l[2];
	...
	w.ll = x;
	r.l[0] = bo_bswap_32(w.l[1]);
	r.l[1] = bo_bswap_32(w.l[0]);
	return r.ll;
}
...
# define be2me_32(x) (x)
# define be2me_64(x) (x)
# define le2me_32(x) bo_bswap_32(x)
# define le2me_64(x) bo_bswap_64(x)
...
#else /* IS_BIG_ENDIAN */
# define be2me_32(x) bo_bswap_32(x)
# define be2me_64(x) bo_bswap_64(x)
# define le2me_32(x) (x)
# define le2me_64(x) (x)
